...
import sys
import urllib3
from passwork_client.passwork_client import PassworkClient
...
def get_client_from_args(args):
    ...
    verify_ssl = getattr(args, 'ssl_verify', True)
    if verify_ssl is None:
        # Deprecated alias: keep behavior aligned with boolean --ssl-verify.
        if getattr(args, "no_ssl_verify", False):
            print(
                "Warning: --no-ssl-verify is deprecated; use --ssl-verify.",
                file=sys.stderr,
            )
            verify_ssl = False
        else:
